...
import { getLeaderKnowledgeAssetArchiveManifest } from '../../shared/api/leader';
...
import '../proofs/proof-preview.css';

const TEXT = {
  loading: '正在加载压缩包内容...',
  loadFailedTitle: '压缩包内容加载失败',
  loadFailedDescription: '暂时无法读取压缩包内文件，请稍后重试。',
  back: '返回上一页',
  title: '压缩包预览',
  hint: '这里展示的是知识库压缩包内的文件列表，点击后会按文件类型直接预览或下载。',
...
export function KnowledgeAssetArchivePage() {
  const navigate = useNavigate();
  const { assetId = '' } = useParams();
  const [keyword, setKeyword] = useState('');

  const manifestQuery = useQuery({
    queryKey: ['knowledge-asset-archive', assetId],
    queryFn: () => getLeaderKnowledgeAssetArchiveManifest(assetId),
                ...
                <div className="proof-archive-entry__row">
                  <div className="proof-archive-entry__meta">
                    <Typography.Title level={5} style={{ marginBottom: 6 }}>
                      <Typography.Link href={resolveAppAwareUrl(entry.previewUrl)} target="_blank" rel="noreferrer">
                        {entry.name}
                      </Typography.Link>
                    </Typography.Title>
                  ...
                  <Space wrap size={[8, 8]} className="proof-archive-entry__actions">
                    <Button
                      type="primary"
                      icon={<EyeOutlined />}
                      href={resolveAppAwareUrl(entry.previewUrl)}
                      target="_blank"
                      rel="noreferrer"
                    >
                      {TEXT.preview}
